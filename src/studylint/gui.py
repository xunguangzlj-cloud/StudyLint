...
def check_to_html(
    notes_path: Path, source_inputs: Path | list[Path]
...
def main() -> None:
    ...
    ttk.Label(notes_tab, text="课程资料").grid(row=1, column=0, sticky="w", pady=9)
    ...
    def choose_source_files() -> None:
        selected = filedialog.askopenfilenames(
            title="选择课程资料",
            filetypes=[
                ("支持的资料", "*.pdf *.pptx *.docx *.md *.txt *.srt"),
                ("全部文件", "*.*"),
            ],
        )
        if selected:
    ...
    def run_check() -> None:
        ...
        try:
            output, counts = check_to_html(notes_path, source_inputs)
    ...
    ).grid(row=2, column=0, columnspan=4, sticky="ew", pady=(24, 14))
    ...
    ttk.Label(
        ...
        foreground="#656d76",
    ).grid(row=4, column=0, columnspan=4, sticky="w", pady=(26, 0))
