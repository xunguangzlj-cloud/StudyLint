...
StudyLint 是一个面向AI学习场景的证据核查工具。把AI整理的笔记和可信课程材料交给它，它会检查失效引用、页码与结论不匹配、直接引语错误和冲突定义；把AI生成的参考文献交给它，它会批量核验开放元数据并提供人工检索入口。
...
`ST003`检查直接引语，`ST006`检查结论与指定位置的文本支持度。没有来源标注本身不会触发问题。StudyLint不会凭空判断世界知识真假：只有与用户提供的可信材料对照后，才能指出确定错误或证据不足。证据推荐只代表文本相关，不代表来源已经支持该结论。
